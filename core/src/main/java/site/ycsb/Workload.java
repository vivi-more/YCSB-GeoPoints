 ...
 * allows the Client to proceed from multiple clients on different machines, in
 * case
 * the client is the bottleneck. For example, if we want to load 1 million
 * records from
 * 2 machines, the first machine should have insertstart=0 and the second
 * insertstart=500000. Additionally,
 * the "insertcount" property, which is interpreted by Client, can be used to
 * tell each instance of the
...
public abstract class Workload {
   ...
   * is specific
   * to one thread. To be clear, this means the returned object should be created
   * anew on each
   * call to initThread(); do not return the same object multiple times.
   * The returned object will be passed to invocations of doInsert() and
   * doTransaction()
   * for this thread. There should be no side effects from this call; all state
   * should be encapsulated
   * in the returned object. If you have no state to retain for this thread,
   * return null. (But if you have
   * no state to retain for this thread, probably you don't need to override
   * initThread().)
   * 
   * @return false if the workload knows it is done for this thread. Client will
   *         terminate the thread.
   *         Return true otherwise. Return true for workloads that rely on
   *         operationcount. For workloads that read
   *         traces from a file, return true when there are more to do, false when
   *         you are done.
   */
   ...
   * effects other than DB operations and mutations on threadstate. Mutations to
   * threadstate do not need to be
